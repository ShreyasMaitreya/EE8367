...
// Function to sort half of the array
void* Sort_Array(void* arg)
{
    long int* arr = (long int*)arg;
    Bucket_Sort(arr, ARRAY_SIZE / 2);
    return NULL;
}
...
int main()
{
    ...
    // Creating threads and sorting half of the array in each thread
    pthread_create(&thread1, NULL, Sort_Array, arr);
    pthread_create(&thread2, NULL, Sort_Array, arr + ARRAY_SIZE / 2);
